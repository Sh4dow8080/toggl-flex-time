...
- The web dashboard is a React/Vite app in `web/` that gets built into a single HTML file and embedded as a TypeScript string constant in `src/generated/dashboard-html.ts`. After changing web code, run `bun run build:web` to regenerate it. This file is gitignored — after a fresh clone, you must build it before the CLI works.
- Toggl API auth is Basic auth with `base64(token:api_token)` where "api_token" is the literal password string.
- Toggl API uses exclusive end dates — the code adds +1 day to the requested end date in `src/toggl.ts`.
- Running timers (negative `duration` from Toggl) are silently filtered out during entry simplification.
